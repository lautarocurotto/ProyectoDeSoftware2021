...
class usuario_tiene_rol(db.Model):
    ...
    def find_by_id(
        cls, id1, rol
    ):  # devuelve la cantidad de administradores de un usuario (0 o 1)
        return cls.query.filter_by(usuario_id=id1, rol_id=rol).count()

    @classmethod
    def find_by_id2(
        cls, id1
    ):  # devuelve la cantidad de operadores de un usuario (0 o 1)
        return cls.query.filter_by(usuario_id=id1, rol_id=1).count()

    @classmethod
    def find_by_id_lista(
        cls, id1
    ):  # devuelve una lista con todos los roles de un usuario
